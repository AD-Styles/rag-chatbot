...
<br>&emsp;최종적으로 단발성 질의응답 시스템에 Memory 모듈을 도입하고 Gradio UI를 통해 시각화함으로써, '기술'을 실제 사용자가 편리하게 사용할 수 있는 '서비스'의 형태로 구현하는 과정을 경험했습니다. RAG 시스템의 성능은 모델의 파라미터만큼이나 이를 제어하는 엔지니어의 '프롬프트 설계 능력'이 최종 서비스의 품질을 결정한다는 사실을 체감했습니다. 이는 향후 실무 솔루션 개발에 있어 중요한 지표가 될 것이라고 생각합니다.
