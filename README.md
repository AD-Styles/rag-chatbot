...
## 💡 회고록 (Retrospective)
이번 RAG(Retrieval-Augmented Generation) 챗봇 프로젝트는 단순한 API 호출을 넘어 데이터 전처리부터 메모리 관리, 배포까지 이어지는 전체 파이프라인을 설계하였습니다. 단순히 LangChain 라이브러리를 사용해 보는 단계를 넘어, LLM의 고질적인 문제인 **'환각(Hallucination)'**과 **'최신 정보 부재'**를 엔지니어링적으로 어떻게 해결할 것인지에 대한 해답을 찾는 과정이었습니다. 우선, 모델의 사전 학습 지식에 의존하지 않고, 외부 문서를 실시간으로 참조하는 구조를 설계하며 LLM의 한계를 극복하려 했습니다. 사용자의 질문이 들어오면 LLM이 바로 답하는 것이 아니라, 관련 문서 조각을 먼저 찾아낸 뒤 그 내용을 기반으로 답변하도록 프롬프트를 구성했습니다.
