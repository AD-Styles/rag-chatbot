...
| **엔드투엔드(E2E) <br>배포** | 백엔드 로직에 Gradio 기반의 직관적인 사용자 인터페이스를 입히고, Hugging Face 플랫폼에 <br>호스팅하여 서비스 가용성 확보 |
...
├── 📄 .gitignore
├── 📄 LICENSE
├── 📄 Maximizing Muscle Hypertrophy.pdf     # RAG 지식 검색 기반 원본 문서
├── 📄 README.md
...
초거대 언어 모델이 처리할 수 없는 방대한 문서를 효율적으로 검색할 수 있도록 데이터를 전처리
...
| 분석 항목&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; | 세부 해결 과정 및 전략 |
| :--- | :--- |
| 환각 현상(Hallucination)<br> 억제 | System Prompt 에 "문서에 없는 내용은 모른다고 답하세요"라는 제약 조건을 명시하여 모델의 추론 범위를 문서 내로 한정 |
...
| :--- | :--- | :--- |
| RAG (검색 증강 생성) | 오픈북 시험 | AI가 사전 지식에만 의존하지 않고, 문서를 참고하여 정답을 도출하는 <br>기법 |
| Vector Embedding | 도서관 십진분류표 | 단어의 의미를 좌표로 변환하여 의미 기반 검색 수행 |
| Text Splitter (Overlap) | 포스트잇 겹쳐 <br>붙이기 | 일부를 겹치게 잘라 문맥을 보존하는 방식 |

---

# 🤖 최종 QA Chatbot 결과물: Huggingface-Space
👉 1. https://huggingface.co/spaces/AD-Styles/RAG_Chatbot
<br>👉 2. https://huggingface.co/spaces/AD-Styles/RAG_Chatbot_Upgrade

---

## 💡 회고록 (Retrospective)
&emsp;&emsp;이번 RAG(Retrieval-Augmented Generation) 챗봇 프로젝트는 단순한 API 호출을 넘어 데이터 전처리부터 메모리 관리, 배포까지 이어지는 전체 파이프라인을 설계하였습니다. 단순히 LangChain 라이브러리를 사용해 보는 단계를 넘어, LLM의 고질적인 문제인 '환각 현상(Hallucination)'과 '최신 정보 부재'를 엔지니어링적으로 어떻게 해결할 것인지에 대한 해답을 지속적으로 생각할 수 있는 과정이었습니다. 우선, 모델의 사전 학습 지식에 의존하지 않고 외부 문서를 실시간으로 참조하는 구조를 설계하며 LLM의 한계를 극복하려 했습니다. 사용자의 질문이 들어오면 LLM이 바로 답하는 것이 아니라, 관련 문서 조각을 먼저 찾아낸 뒤 그 내용을 기반으로 답변하도록 프롬프트를 구성했습니다. 시스템 프롬프트에 "문서에 없는 내용은 모른다고 답하라"는 제약 조건을 명시함으로써, 모델이 모르는 것을 지어내는 '환각 현상'을 효과적으로 제어할 수 있었습니다. 이는 AI 서비스의 신뢰성을 결정짓는 가장 중요한 엔지니어링 포인트였습니다.
<br>&emsp;&emsp;그리고 방대한 PDF 문서를 컴퓨터가 이해할 수 있는 벡터(Vector) 형태로 변환하고 저장하는 과정에서, 텍스트를 너무 작게 자르면 의미가 단절되고, 너무 크게 자르면 검색 노이즈가 섞인다는 문제를 해결하기 위해서 1,000자 단위로 분할하되, 조각 간 200자의 Overlap을 설정했습니다. 적절한 오버랩을 통해 잘린 문장 사이의 문맥적 연결 고리를 보존하는 것이 검색 품질에 직결됨을 확인했습니다. 또한, 단발성 질의응답을 넘어 사용자와의 이전 대화 흐름을 기억하는 메모리 시스템을 LCEL 파이프라인에 통합했습니다. InMemoryChatMessageHistory와 RunnableWithMessageHistory를 결합하여 세션별 대화 기록을 관리했습니다. MessagesPlaceholder를 사용해 프롬프트 내에 chat_history가 삽입될 위치를 지정함으로써, 질문이 이전 맥락과 연결될 때(예: "그거 좀 더 자세히 설명해줘") 모델이 정확한 지칭 대상을 인지하도록 설계했습니다. 최종적으로 단발성 질의응답 시스템에 Memory 모듈을 도입하고 Gradio UI를 통해 시각화함으로써, '기술'을 실제 사용자가 편리하게 사용할 수 있는 '서비스'의 형태로 구현하는 과정을 경험했습니다.
<br>&emsp;&emsp;이번 프로젝트를 통해 데이터 로딩 → 벡터화 → 검색 → 생성 → 메모리 관리로 이어지는 RAG의 전 과정을 코드로 직접 제어하며 시스템 아키텍트로서의 감각을 익혔습니다. 각 파라미터(chunk_overlap, temperature)가 서비스의 최종 품질에 미치는 영향을 데이터로 확인한 것이 가장 큰 수확입니다. 특히 RAG 시스템의 성능은 모델의 크기만큼이나 이를 제어하는 엔지니어가 데이터를 어떻게 가공하고(Chunking), 어떻게 질문을 던지느냐(Prompt Engineering) 에 따라 최종 서비스의 품질이 결정된다는 사실을 깨달았습니다.
