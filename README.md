...
## 💡 회고록 (Retrospective)
이번 RAG(Retrieval-Augmented Generation) 챗봇 프로젝트는 단순한 API 호출을 넘어 데이터 전처리부터 메모리 관리, 배포까지 이어지는 전체 파이프라인을 설계하였습니다. 단순히 LangChain 라이브러리를 사용해 보는 단계를 넘어, LLM의 고질적인 문제인 **'환각 현상(Hallucination)'**과 **'최신 정보 부재'**를 엔지니어링적으로 어떻게 해결할 것인지에 대한 해답을 찾는 과정이었습니다. 우선, 모델의 사전 학습 지식에 의존하지 않고, 외부 문서를 실시간으로 참조하는 구조를 설계하며 LLM의 한계를 극복하려 했습니다. 사용자의 질문이 들어오면 LLM이 바로 답하는 것이 아니라, 관련 문서 조각을 먼저 찾아낸 뒤 그 내용을 기반으로 답변하도록 프롬프트를 구성했습니다. 시스템 프롬프트에 "문서에 없는 내용은 모른다고 답하라"는 제약 조건을 명시함으로써, 모델이 모르는 것을 지어내는 '환각 현상'을 효과적으로 제어할 수 있었습니다. 이는 AI 서비스의 신뢰성을 결정짓는 가장 중요한 엔지니어링 포인트였습니다.
<br>&emsp;그리고 방대한 PDF 문서를 컴퓨터가 이해할 수 있는 벡터(Vector) 형태로 변환하고 저장하는 과정에서, 텍스트를 너무 작게 자르면 의미가 단절되고, 너무 크게 자르면 검색 노이즈가 섞인다는 문제를 해결하기 위해서 적절한 오버랩을 통해 잘린 문장 사이의 문맥적 연결 고리를 보존하는 것이 검색 품질에 직결됨을 확인했습니다. 코사인 유사도 연산을 통해 질문과 가장 닮은 벡터 좌표를 찾아내는 과정은 '의미 기반 검색'의 강력함을 체감하게 해주었습니다.
