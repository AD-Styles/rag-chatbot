...
<br>&emsp;그리고 방대한 PDF 문서를 컴퓨터가 이해할 수 있는 벡터(Vector) 형태로 변환하고 저장하는 과정에서, 텍스트를 너무 작게 자르면 의미가 단절되고, 너무 크게 자르면 검색 노이즈가 섞인다는 문제를 해결하기 위해서 1,000자 단위로 분할하되, 조각 간 200자의 Overlap을 설정했습니다. 적절한 오버랩을 통해 잘린 문장 사이의 문맥적 연결 고리를 보존하는 것이 검색 품질에 직결됨을 확인했습니다. 또한, 단발성 질의응답을 넘어 사용자와의 이전 대화 흐름을 기억하는 메모리 시스템을 LCEL 파이프라인에 통합했습니다.
