...
<br>&emsp;그리고 방대한 PDF 문서를 컴퓨터가 이해할 수 있는 벡터(Vector) 형태로 변환하고 저장하는 과정에서, 텍스트를 너무 작게 자르면 의미가 단절되고, 너무 크게 자르면 검색 노이즈가 섞인다는 문제를 해결하기 위해서 1,000자 단위로 분할하되, 조각 간 200자의 Overlap을 설정했습니다. 적절한 오버랩을 통해 잘린 문장 사이의 문맥적 연결 고리를 보존하는 것이 검색 품질에 직결됨을 확인했습니다. 또한, 단발성 질의응답을 넘어 사용자와의 이전 대화 흐름을 기억하는 메모리 시스템을 LCEL 파이프라인에 통합했습니다. InMemoryChatMessageHistory와 RunnableWithMessageHistory를 결합하여 세션별 대화 기록을 관리했습니다. MessagesPlaceholder를 사용해 프롬프트 내에 chat_history가 삽입될 위치를 지정함으로써, 질문이 이전 맥락과 연결될 때(예: "그거 좀 더 자세히 설명해줘") 모델이 정확한 지칭 대상을 인지하도록 설계했습니다.

**"어떻게 하면 거대 언어 모델(LLM)이 가진 지식의 시효성과 신뢰성 문제를 엔지니어링적으로 해결할 것인가?"** 에 대한 답을 찾아가는 과정이었습니다. API를 호출하는 단계를 넘어 로더, 분할기, 벡터 DB, 메모리 모듈을 LCEL 파이프라인으로 유기적으로 연결하며 전체적인 데이터 흐름을 제어하는 설계 역량을 길렀습니다. 또한 단발성 질의응답 시스템에 Memory 모듈을 도입하고 Gradio UI를 통해 시각화함으로써, '기술'을 실제 사용자가 편리하게 사용할 수 있는 '서비스'의 형태로 구현하는 과정을 경험했습니다. RAG 시스템의 성능은 모델의 파라미터만큼이나 이를 제어하는 엔지니어의 '프롬프트 설계 능력'이 최종 서비스의 품질을 결정한다는 사실을 체감했습니다. 이는 향후 실무 솔루션 개발에 있어 중요한 지표가 될 것이라고 생각합니다.
