...
<br>&emsp;이번 프로젝트를 통해 데이터 로딩 → 벡터화 → 검색 → 생성 → 메모리 관리로 이어지는 RAG의 전 과정을 코드로 직접 제어하며 시스템 아키텍트로서의 감각을 익혔습니다. 각 파라미터(chunk_overlap, temperature)가 서비스의 최종 품질에 미치는 영향을 데이터로 확인한 것이 가장 큰 수확입니다. 특히 RAG 시스템의 성능은 모델의 크기만큼이나 이를 제어하는 엔지니어가 데이터를 어떻게 가공하고(Chunking), 어떻게 질문을 던지느냐(Prompt Engineering) 에 따라 최종 서비스의 품질이 결정된다는 사실을 체감하였습니다. 
