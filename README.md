...
## 💡 회고록 (Retrospective)
이번 RAG(Retrieval-Augmented Generation) 챗봇 프로젝트는 단순한 API 호출을 넘어 데이터 전처리부터 메모리 관리, 배포까지 이어지는 전체 파이프라인을 설계하였습니다. 단순히 LangChain 라이브러리를 사용해 보는 단계를 넘어, LLM의 고질적인 문제인 **'환각(Hallucination)'**과 **'최신 정보 부재'**를 엔지니어링적으로 어떻게 해결할 것인지에 대한 실전적 해답을 찾는 과정이었습니다.
